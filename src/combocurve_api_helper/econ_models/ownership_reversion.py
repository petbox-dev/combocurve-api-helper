...
# API `ownership` shape: `{"initialOwnership": {...}, "firstReversion": null|{...},
# "secondReversion": ..., ..., "twentiethReversion": ...}` -- ALL 20 reversion keys are always
# present on a model, set to `null` when that tier is unused (models with only 1 active reversion
# still carry `secondReversion` through `twentiethReversion` as explicit `null`, not omitted).
# The ordinal word IS both the API key prefix (`firstReversion`) and the CSV `Key` column value
# (`first`): `Key='initial'` for `initialOwnership`, `Key='first'` for `firstReversion`.
_REVERSION_ORDINALS = [
    'first',
    'second',
    'third',
...
#
# Only three `reversionType` values are mapped: 'PayoutWithoutInvestment', 'PayoutWithInvestment',
# and 'Date'. ComboCurve's schema lists a larger enum (`Date`, `AsOf`, `WhCumOil`, `WhCumGas`,
# `WhCumBoe`, `Irr`, `PayoutWithInvestment`, `PayoutWithoutInvestment`, `UndiscRoi`) -- anything
# outside the three mapped values raises NotImplementedError rather than guessing at its CSV
# rendering.
#
# 'PayoutWithInvestment' -> 'poi': the field list for `PayoutWithInvestment` and
# `PayoutWithoutInvestment` is otherwise IDENTICAL -- the type name carries all the semantic
# difference. 'poi' extends the existing 'po' ("payout") abbreviation convention with a distinct,
# round-trippable token; if an export ever surfaces a different CSV token for this type, update
# this mapping (and the token below) to match.
...
#
# There is a second `reversionTiedTo` shape -- `{"type": "date", "value": "<ISO date>"}`
# (`PayoutWithInvestment` tiers appear to always be tied to a date; a `Date`-type tier can have a
# tied-to date that differs from the tier's own `reversionValue` date). Unlike the as_of/absent
# case, this shape IS fully round-trippable: it renders as the formatted date itself (via
# `to_csv_date`/`from_csv_date`, matching the date-column convention used elsewhere in this
# package, e.g. `capex.py`), which is unambiguously distinguishable from the literal `"as of"`
...
class InitialOwnershipData(BaseModel):
    """`ownership.initialOwnership`: `{"workingInterest": 100, "netProfitInterestType": "expense",
    ...
    `netProfitInterest` value but never a `netProfitInterestType` key; the CSV's 'NPI Type'
    column is therefore populated only on the 'initial' row.
...
class ReversionTierData(BaseModel):
    """One `ownership.<n>Reversion` element: `{"reversionType": "PayoutWithoutInvestment",
    "reversionValue": 12290000, "balance": "gross", "includeNetProfitInterest": "yes",
    "workingInterest": 0.177749, "netRevenueInterest": 0.132697, "leaseNetRevenueInterest":
    75.00140853, "netProfitInterest": 0, "oilNetRevenueInterest": null, "gasNetRevenueInterest":
    null, "nglNetRevenueInterest": null, "dripCondensateNetRevenueInterest": null}` (+ optional
    `reversionTiedTo`, see `ReversionTiedTo`/`_REVERSION_TIED_TO_CSV_DEFAULT`).

    `includeNetProfitInterest` is an API STRING ('yes'/'no'), not a bool, so it passes through to
    the CSV 'Include NPI' column unchanged, unlike the `formats.yes_no`-style bool columns
    ...
    threshold; for `reversionType == 'Date'` it is a plain ISO `"YYYY-MM-DD"` STRING (CC's schema
    may present it as a full ISO datetime like `"2023-04-11T00:00:00.000Z"` --
    `formats.to_csv_date`/`from_csv_date`, via `datetime.fromisoformat`, accept both forms
    transparently). Maps to the CSV 'Reversion Value' column as a formatted `MM/DD/YYYY` date for
    `'Date'` tiers (same date-column convention as `capex.py`), else as a plain number.

    `balance`/`includeNetProfitInterest` are typed `Optional[str]` for pydantic permissiveness,
    but on `'Date'`-type tiers (where neither concept applies) they are present as the EMPTY
...
def _reversion_value_to_csv(tier: ReversionTierData) -> str:
    """'Reversion Value' cell: a formatted date for `'Date'` tiers (`reversionValue` is an ISO
    date STRING for this type), else a plain number. The `isinstance` checks double as a
    fail-loud guard against an unexpected type/value combination (e.g. a numeric 'Date' value).
    """
...
def _tied_to_to_csv(tied_to: Optional[ReversionTiedTo]) -> str:
    """'Reversion Tied To' cell. See `_REVERSION_TIED_TO_CSV_DEFAULT` for the as_of/absent
    collapse and the date+value rendering.
    """
    if tied_to is None:
...
class OwnershipReversionMapper(EconModelMapper):
    ...
    def _initial_from_csv(row: Dict[str, str]) -> Dict[str, Any]:
        ...
        # NO exclude_none: every initialOwnership field is always present, even when null (see
        # InitialOwnershipData docstring) -- exclude_none would wrongly drop a genuinely-null
        # phase-NRI key instead of reproducing it as `null`.
    ...
    def _reversion_from_csv(row: Dict[str, str]) -> Dict[str, Any]:
        ...
        data = ReversionTierData(
            ...
            # On 'Date'-type tiers (where neither concept applies),
        ...
        if data.reversion_tied_to is not None:
            ...
            # was never present; `type` is required (never `None`) so it always survives.

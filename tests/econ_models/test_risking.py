...
def test_to_row_dicts_emits_exactly_five_phase_rows_in_order() -> None:
    rows = RiskingMapper().to_row_dicts(SAMPLE_WELL_1_P50)
    assert len(rows) == 5
...
def test_to_row_dicts_int_value_stays_integral() -> None:
    rows = RiskingMapper().to_row_dicts(SAMPLE_WELL_1_P50)
    ngl = next(r for r in rows if r['Phase'] == 'ngl')
    assert ngl['Value'] == '100'  # not '100.0'


def test_to_row_dicts_risk_prod_none_defaults_to_yes() -> None:
    rows = RiskingMapper().to_row_dicts(LOW_PROPPANT)
    assert len(rows) == 5
    for r in rows:
        assert r['Risk Hist Prod'] == 'yes'
        assert r['Risk NGL & Drip Cond via Gas Risk'] == 'yes'
...
def test_to_row_dicts_risk_prod_false_renders_no() -> None:
    rows = RiskingMapper().to_row_dicts(SAMPLE_UNIT_2)
    assert len(rows) == 5
    for r in rows:
...
def test_roundtrip_exact_sample_well_1() -> None:
    m = RiskingMapper()
    rebuilt = m.from_row_dicts(m.to_row_dicts(SAMPLE_WELL_1_P50))
...
def test_roundtrip_risk_prod_false_exact() -> None:
    m = RiskingMapper()
    rebuilt = m.from_row_dicts(m.to_row_dicts(SAMPLE_UNIT_2))
...
def test_roundtrip_risk_prod_none_collapses_to_explicit_true() -> None:
    ...
    m = RiskingMapper()
    rebuilt = m.from_row_dicts(m.to_row_dicts(LOW_PROPPANT))
...
def test_from_row_dicts_ignores_wells_rows() -> None:
    ...
    # RiskingMapper's class docstring. from_row_dicts must skip it without error.
    m = RiskingMapper()
    rows = m.to_row_dicts(SAMPLE_WELL_1_P50)
...
def test_unknown_entire_well_life_raises() -> None:
    ...
    with pytest.raises(NotImplementedError):
        RiskingMapper().to_row_dicts(model)
...
def test_unknown_criteria_raises_on_inverse() -> None:
    ...
    rows[0]['Criteria'] = 'fpd'
    with pytest.raises(NotImplementedError):
        RiskingMapper().from_row_dicts(rows)
...
def test_to_row_dicts_unique_model_type() -> None:
    ...
    rows = RiskingMapper().to_row_dicts(model)
